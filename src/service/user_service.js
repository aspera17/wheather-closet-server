...
//login,sign 할 껀데!!? 왜 getUserToken로 변수했는지? 다른 변수 이름생각해보자.
const getUserToken = async (email, password) => {
    ...
    // accessToken 생성(<= login-required)
    const secretKey = process.env.JWT_SECRET_KEY;
    const accessToken = jwt.sign({ sub: getUserId }, secretKey);
    // refreshToken 생성 => 임의의 난수 생성(API Key, 토큰에는 uuid, uuid.v4)
    const refreshToken = uuidv4();

    await models.user_token.create({ refresh_token: refreshToken  , user_id: getUserId})

    return  {"acessToken": accessToken, "refreshToken": refreshToken} ;
}
...
const delUser = async(userId) => {
    ...
    const deleteAt = await models.user.update({'deleted_at': Date },{where: {user_id: userId}})
    return  ;
}
